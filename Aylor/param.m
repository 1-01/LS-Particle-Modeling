function [h, d, h0, z0, zw, L, ustar, v_s, k, np, ...
    zmin, zmax, tmax, Q_LS, t0, deltstart, sigu_zw, ...
    sigw_zw, sigu_h, sigw_h, ex_h, psi_h, Ubar_h, T_Lh, ...
    T_L0, beta, x_sense] ...
    = param(experiment)
...
h0 = h0(:,experiment);
ustar = ustar(experiment);
L = L(experiment);
z0 = z0(experiment);
x_sense = x_sense(experiment);
...
v_s = .019;  % 0.019 from 2001 paper for diameter of lycopodium spores // (3*10^-3*Dp^2)/100 (m/s) approximates settling velocity from Aylor's text eq. 3.13
